...
const SocketConnection = () => {
    ...
    useEffect(() => {
        const loadPrevNotification = async () => {
            try {
                const unseenNotificaton = await fetch(`${VITE_SERVER_URL}/api/notification/${currentUser._id}`);
    ...
    const sendNotificationToDB = async (notificationData) => {
        try {
            const sendNotification = await fetch(`${VITE_SERVER_URL}/api/notification/create`, {
                method: 'POST',
