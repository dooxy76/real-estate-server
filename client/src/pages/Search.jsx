...
const Search = () => {
    ...
    useEffect(() => {
        (async () => {
            try {
                setLoading(true)
                const res = await fetch(`${VITE_SERVER_URL}/api/posts?searchTerm=${searchTermState}&type=${formState.type}&parking=${formState.parking}&furnished=${formState.furnished}&offer=${formState.offer}&page=${pageCount}`)
