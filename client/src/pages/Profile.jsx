...
const Profile = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`${VITE_SERVER_URL}/api/users/update/${currentUser._id}`, {
        method: 'POST',
  ...
  const handleDelete = async () => {
    try {
      dispatch(loddingStart())
      const res = await fetch(`${VITE_SERVER_URL}/api/users/delete/${currentUser._id}`, {
        method: 'DELETE'
      })
  ...
  useEffect(() => {
    const loadPost = async () => {
      try {
        ...
        const res = await fetch(`${VITE_SERVER_URL}/api/users/posts/${currentUser._id}`)
        const data = await res.json();
        if (data.success === false) {
          toast.error(data.message, {
            autoClose: 2000,
  ...
  const handlePostDelete = async (postId) => {
    try {
      const res = await fetch(`${VITE_SERVER_URL}/api/posts/delete/${postId}`, {
        method: 'DELETE',
      })
      const data = await res.json();
